...
namespace gr {
  namespace azure_software_radio {
    ...
    template <class T>
    typename difi_source_cpp<T>::sptr difi_source_cpp<T>::make(std::string ip_addr, uint32_t port, int stream_number, int bit_depth, int context_pkt_behavior)
    {
      return gnuradio::make_block_sptr<difi_source_cpp_impl<T>>(ip_addr, port, stream_number, bit_depth, context_pkt_behavior);
    }

    template <class T>
    difi_source_cpp_impl<T>::difi_source_cpp_impl(std::string ip_addr, uint32_t port, int stream_number, int bit_depth, int context_pkt_behavior)
